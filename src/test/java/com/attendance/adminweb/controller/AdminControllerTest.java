...
class AdminControllerTest {
    ...
    @MockBean
    private BackendAdminUserDetailsService backendAdminUserDetailsService;
    ...
    void dashboardShouldRenderForAuthenticatedUser() throws Exception {
        given(adminService.resolveRequestedWorkplaceId(anyString(), nullable(Long.class))).willReturn(null);
        given(adminService.normalizeDashboardFilter(nullable(String.class))).willReturn("ALL");

        mockMvc.perform(get("/dashboard"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/app/dashboard.html?filter=ALL"));
    }
    ...
    void sqlConsoleShouldRenderForAdmin() throws Exception {
        ...
        mockMvc.perform(get("/sql-console"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/app/sql-console.html"));
    }
    ...
    void sqlConsoleExecuteShouldRenderResult() throws Exception {
        ...
        mockMvc.perform(post("/sql-console/query-data")
                        .param("queryText", "select 1")
                        .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith("application/json"))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("\"queryText\":\"select 1\"")))
                .andExpect(content().string(org.hamcrest.Matchers.containsString("\"truncated\":false")));
    }
